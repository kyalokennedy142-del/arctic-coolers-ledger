...
const PurchasesPage = () => {
  ...
  // ✅ SAFE: Handle undefined/null values
  const productsTotal = rows.reduce((sum, row) => sum + (Number(row.amount) || 0), 0);
  const totalExpenditure = productsTotal + (Number(formData.transportCost) || 0);
  ...
  // ✅ SAFE: Stats calculations with null checks
  const totalPurchases = purchases?.length || 0;
  const totalSpent = purchases?.reduce((sum, p) => sum + (p.totalExpenditure || 0), 0) || 0;
  const totalTransport = purchases?.reduce((sum, p) => sum + (p.transportCost || 0), 0) || 0;
    ...
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
        ...
        <section className="space-y-4">
          {purchases?.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-xl shadow-sm ring-1 ring-gray-100">
              ...
              <button onClick={() => setShowForm(true)} className="mt-4 text-orange-600 font-medium">
                Add your first purchase →
              </button>
            </div>
          ) : (
            purchases.map((purchase) => (
              <div key={purchase.id} className="rounded-xl bg-white shadow-sm ring-1 ring-gray-100 overflow-hidden">
                {/* Card Header */}
                <div className="flex flex-col md:flex-row md:items-center justify-between p-4 md:p-6 gap-4">
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto">
                    <div className="text-right">
                      ...
                      {/* ✅ SAFE: Handle undefined with fallback */}
                      <p className="text-lg font-bold text-orange-600">
                        KSh {(purchase.totalExpenditure || 0).toFixed(2)}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 border-l border-gray-100 pl-4">
                      <button onClick={() => handleEdit(purchase)} className="text-gray-400 hover:text-orange-500">
                        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                      </button>
                      <button onClick={() => handleDelete(purchase.id)} className="text-gray-400 hover:text-red-500">
                        <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
                      </button>
                      <button onClick={() => setExpandedId(expandedId === purchase.id ? null : purchase.id)} className="text-gray-400 hover:text-gray-600">
                        <svg className={`h-5 w-5 transition-transform ${expandedId === purchase.id ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex justify-end">
                      <div className="w-full md:w-64 bg-white rounded-lg p-3 space-y-2 shadow-sm border border-gray-100">
                        <div className="flex justify-between text-sm text-gray-600">
                          ...
                          <span>KSh {(purchase.items?.reduce((sum, i) => sum + (i.amount || 0), 0) || 0).toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-sm text-gray-600">
                          <span>Transport</span>
                          <span>KSh {(purchase.transportCost || 0).toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-sm font-bold text-orange-700 pt-2 border-t border-gray-100">
                          <span>TOTAL</span>
                          <span>KSh {(purchase.totalExpenditure || 0).toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="fixed inset-0 bg-black/50" onClick={handleCloseForm} />
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative w-full max-w-3xl rounded-2xl bg-white shadow-xl">
              {/* Modal Header */}
              <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                <h2 className="text-lg font-semibold text-gray-800">{editingPurchase ? 'Edit Purchase' : 'New Purchase'}</h2>
                <button onClick={handleCloseForm} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100">
                  <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="max-h-[70vh] overflow-y-auto p-6 space-y-8">
                {/* Header Fields */}
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Purchase Header</p>
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-gray-700">Date *</label>
                      <input type="date" name="date" value={formData.date} onChange={handleFormChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none" />
                    </div>
                    <div className="space-y-1">
                      <label className="text-xs font-semibold text-gray-700">Agent</label>
                      <select name="agent" value={formData.agent} onChange={handleFormChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none bg-white">
                    ...
                    <div className="space-y-1">
                      ...
                      <select name="company" value={formData.company} onChange={handleFormChange} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none bg-white" required>
                ...
                {/* Items Entry */}
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Bottles Entry</p>
                  <div className="space-y-3">
                    ...
                    {rows.map((row) => (
                      <div key={row.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center bg-gray-50/50 p-2 rounded-lg">
                        <div className="col-span-1 md:col-span-5">
                          <select value={row.productType} onChange={(e) => handleRowChange(row.id, 'productType', e.target.value)} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none bg-white">
                  ...
                  <button onClick={addRow} className="mt-4 flex items-center rounded-lg border border-orange-200 bg-orange-50 px-4 py-2 text-sm font-medium text-orange-700 hover:bg-orange-100">
              ...
              <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-t border-gray-100 px-6 py-4 bg-gray-50 rounded-b-2xl">
                <div className="bg-white rounded-xl p-4 w-full md:w-64 space-y-2 shadow-sm border border-gray-100">
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>Products Total</span>
                    <span>KSh {(productsTotal || 0).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>Transport</span>
                    <span>KSh {(Number(formData.transportCost) || 0).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-base font-bold text-orange-700 pt-2 border-t border-gray-200">
                    <span>TOTAL</span>
                    <span>KSh {(totalExpenditure || 0).toFixed(2)}</span>
                  </div>
                </div>
                <div className="flex gap-3 w-full md:w-auto">
                  <button onClick={handleCloseForm} className="flex-1 md:flex-none rounded-lg border border-gray-200 bg-white px-6 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</button>
                  <button onClick={handleSave} className="flex-1 md:flex-none rounded-lg bg-orange-500 px-6 py-2.5 text-sm font-semibold text-white hover:bg-orange-600">
