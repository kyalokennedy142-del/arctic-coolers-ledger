...
const CustomersPage = () => {
  ...
  // ✅ FIXED: Handle form submission with toast
  ...
  // ✅ FIXED: Handle delete with toast
  ...
  const customersFullyPaid = customers?.filter((c) => {
    const totalCredit = (c.transactions || []).filter((t) => t.type === 'Credit').reduce((sum, t) => sum + (t.amount || 0), 0);
    const totalPaid = (c.transactions || []).filter((t) => t.type === 'Payment').reduce((sum, t) => sum + (t.paid || 0), 0);
    return totalCredit - totalPaid <= 0;
  }).length || 0;
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            ...
            <p className="mt-1 text-2xl font-bold text-blue-600">{totalCustomers}</p>
          </div>
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            <p className="text-sm font-medium text-slate-500">With Outstanding Balance</p>
            <p className="mt-1 text-2xl font-bold text-amber-600">{customersWithBalance}</p>
          </div>
          <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
            <p className="text-sm font-medium text-slate-500">Fully Paid</p>
            <p className="mt-1 text-2xl font-bold text-green-600">{customersFullyPaid}</p>
          </div>
        </div>
                      ...
                      <tr key={customer.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <span className={`font-medium ${balance > 0 ? 'text-amber-600' : balance < 0 ? 'text-green-600' : 'text-slate-400'}`}>
                            {balance > 0 ? `KSh ${(balance || 0).toFixed(2)}` : balance < 0 ? `KSh ${(balance || 0).toFixed(2)}` : 'KSh 0.00'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
