...
import { formatKSH, formatPhoneForWhatsApp, formatPhoneForDisplay, getInactivityBadge } from '../../utils/formatCurrency';  // ✅ ../../ not ../
...
// ─────────────────────────────────────────────────────────────
// Customer Form Component - ✅ Uses phone (not payment_name)
// ─────────────────────────────────────────────────────────────
const CustomerForm = ({ customer, onSuccess, onCancel }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Contact (Email/Alternate)</label>
        <input
          type="text"
...
// ─────────────────────────────────────────────────────────────
// Customer Card Component - ✅ Shows phone in 07XX format
// ─────────────────────────────────────────────────────────────
const CustomerCard = ({ customer, transactions, onEdit, onAddTransaction, onWhatsApp, onClick }) => {
  ...
  // ✅ Get phone from phone field (formatted as 07XX)
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${badgeColors[badge.color]}`}>
              {badge.label}
            </span>
          </div>
          ...
          <div className="mt-2 space-y-1 text-sm">
            ...
            {phoneNumber && (
              <p className="text-gray-600">📱 {formatPhoneForDisplay(phoneNumber, 'local')}</p>
            )}
        ...
        <div className="flex flex-col gap-2">
          <button
            onClick={(e) => { e.stopPropagation(); onWhatsApp(phoneNumber); }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); onAddTransaction(customer); }}
...
// ─────────────────────────────────────────────────────────────
const GroupHeader = ({ label, color, count, customers }) => {
  const handleCopy = () => {
    const list = customers.map(c => `${c.name} - ${formatPhoneForDisplay(c.phone, 'local') || 'No phone'}`).join('\n');
  ...
  return (
    <div className={`flex items-center justify-between p-3 rounded-lg border ${colorClasses[color]}`}>
      ...
      <button
        onClick={handleCopy}
        disabled={count === 0}
        className="px-3 py-1.5 bg-white text-sm font-medium rounded-lg hover:bg-gray-50 disabled:opacity-50"
      >
        📋 Copy List
      </button>
    </div>
  ...
  const customersWithDetails = useMemo(() => {
    ...
    return customers.map(customer => {
      ...
      return {
        ...
        lastTransaction: lastTxDate ? new Date(lastTxDate).toISOString() : null
      };
    });
  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <button onClick={refresh} className="px-4 py-2 bg-blue-600 text-white rounded-lg">Retry</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <button
          ...
          className="w-full mb-6 bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 rounded-lg"
        >
          + Add Customer
        </button>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 space-y-4">
          <input
            type="text"
            placeholder="Search by name, phone, contact, or location..."
            value={searchTerm}
          ...
          <div className="flex items-center gap-2">
            ...
            />
            {dateFilter && (
              <button onClick={() => setDateFilter('')} className="text-sm text-blue-600 hover:underline">
                Clear
              </button>
            )}
          </div>
                ...
                <div className="mt-3 space-y-3">
                  {groupCustomers.map(customer => (
                    <CustomerCard
                      ...
                      onAddTransaction={(c) => { setSelectedCustomerForTx(c); setShowTransactionForm(true); }}
                      onWhatsApp={() => handleWhatsApp(customer.phone)}
                      onClick={() => navigate(`/credit-statements?customer=${encodeURIComponent(customer.name)}`)}
                    />
                  ))}
                </div>
              </div>
      ...
      <BottomSheet
        ...
        onClose={() => { setShowTransactionForm(false); setSelectedCustomerForTx(null); }}
        title={`Add Transaction - ${selectedCustomerForTx?.name || ''}`}
      >
        <TransactionForm
          customerId={selectedCustomerForTx?.id}
          customerName={selectedCustomerForTx?.name}
          onSuccess={handleAddTransaction}
          onCancel={() => { setShowTransactionForm(false); setSelectedCustomerForTx(null); }}
        />
      </BottomSheet>
    </div>
  );
